...
pacman::p_load(tidyverse, # for data wrangling
               shiny, shinydashboard, shinythemes, bslib, # for shiny app
...
merge_the_data <- function(file_name) {
  ...
  ### 1) Read in the energy consumption data
  # Define the path directory
  directory <-
    "https://raw.githubusercontent.com/quinnei/Residential-Energy-Consumption/main/RECS-Shiny-App/1_Dataset/"
  # Specify the location of the csv file
  file_path <- paste0(directory, file_name, ".csv")
  # Read the csv dataset
  energy_insecurity <- read.csv(file_path)
  ...
  merged_data <- left_join(energy_insecurity, US, by = c("STATE" = "NAME"))
...
map_energy_consumption <- create_map(
  ...
  # For each state, indicate EUI & total average expenditures on energy
